...
def train(config: ExperimentConfig):
    ...
    predicates_path = config.data_dir / "predicates.csv"
    print("Loading predicates from {}".format(predicates_path))
    assert predicates_path.exists(), f"Predicates file not found: {predicates_path}"
    data_binary_predicates, data_unary_predicates = load_predicates(predicates_path)
    print("{} unary predicates and {} binary predicates in the signature.".format(len(data_unary_predicates),
                                                                                  len(data_binary_predicates)))

    train_graph_path = config.data_dir / "train_graph.tsv"
    print("Loading graph data from {}".format(train_graph_path))
    assert os.path.exists(train_graph_path), f"Train graph file not found: {train_graph_path}"
    train_graph_dataset = data_parser.parse(train_graph_path)
    # TODO: give some info about the loaded graph
    ...
    if config.encoding_scheme == EncoderType.CANONICAL:
        ...
        iclr_encoder_decoder.save_to_file(config.exp_dir / "encoders/iclr22_encoder.tsv")
    ...
    can_encoder_decoder.save_to_file(config.exp_dir / 'encoders/canonical_encoder.tsv')
    ...
    train_examples_path = config.data_dir / "train_examples.tsv"
    print("Loading graph data from {}".format(train_examples_path))
    assert os.path.exists(train_examples_path), f"Train examples file not found: {train_examples_path}"
    ...
    checkpoints_folder = config.exp_dir / "models/checkpoints"
    ...
    torch.save(model, config.exp_dir / "models/model.pt")
